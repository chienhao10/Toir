...
function Karthus:KarthusMenu()
  ...
  self.Use_Combo_R = self:MenuBool("使用 R",true)
  self.Rsafe = self:MenuSliderInt("安全距离 R", 900)
  self.Draw_Rs = self:MenuBool("显示手动 R 提示",true)
  self.Use_Auto_Q = self:MenuBool("自动 Q",true)
  self.AQMana = self:MenuSliderInt("自动 Q 蓝量", 40)
  self.Use_Auto_E = self:MenuBool("自动 E",true)
  ...
  self.Evade_R = self:MenuBool("R 时不躲避",true)
  self.AntiGapclose = self:MenuBool("防突进 W",true)
...
function Karthus:OnDrawMenu()
  if Menu_Begin(self.menu) then
    ...
    if Menu_Begin("自动菜单") then
      self.Use_Auto_Q = Menu_Bool("自动 Q",self.Use_Auto_Q,self.menu)
      self.AQMana = Menu_SliderInt("自动 Q 蓝量", self.AQMana, 0, 100, self.menu)
   ...
   if Menu_Begin("杂项菜单") then
      self.Evade_R = Menu_Bool("R 时不躲避",self.Evade_R,self.menu)
      self.AntiGapclose = Menu_Bool("防突进 W",self.AntiGapclose,self.menu)
      self.Draw_Rs = Menu_Bool("显示手动 R 提示",self.Draw_Rs,self.menu)
      self.Rsafe = Menu_SliderInt("安全距离 R", self.Rsafe, 0, 9000, self.menu)
      Menu_End()
      end
